...
-- Step 1: Create a temporary column to store the current hr_id values
ALTER TABLE employees ADD COLUMN temp_hr_id VARCHAR(50);

-- Step 2: Copy existing hr_id values to temp column
UPDATE employees SET temp_hr_id = hr_id;

-- Step 3: Drop the old hr_id column
ALTER TABLE employees DROP COLUMN hr_id;

-- Step 4: Create new hr_id column as SERIAL
ALTER TABLE employees ADD COLUMN hr_id SERIAL;

-- Step 5: Update the sequence to start from the maximum existing value + 1
